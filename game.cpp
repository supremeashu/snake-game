...
public:
    ...
    {
        if ((direction == SNAKE_LEFT && dir == SNAKE_RIGHT) || (direction == SNAKE_RIGHT && dir == SNAKE_LEFT) || (direction == SNAKE_TOP && dir == SNAKE_BOTTOM) ||
...
public:
    ...
    void setGridSnake()
    {
        ...
        for (int i = 0; i < (int)prevSnake.size(); i++)
        {
    ...
    bool checkGameOver()
    {
        ...
        gameOver = snakeHead.row >= GRID_HEIGHT - 1 || snakeHead.row <= 0 || snakeHead.col <= 0 ||
...
// Function to print the instructions and get the difficulty level from the user
